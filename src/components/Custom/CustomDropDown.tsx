  ...
  const renderFields = () => {
    switch (inputType) {
          ...
          return (
            ...
            >
              <Radio
                ...
                value={item}
                onChange={(value) => {
                  handleInputChange(value.currentTarget.value);
                }}
              />
              <Text marginLeft='0px' fontWeight={400}>
                {t(item)}
              </Text>
            </Grid>
          );
      ...
      case 'date':
        return numOfInputs.map((item, index, array) => {
          return (
            <Grid
              item
              lg={array.length == 2 ? 6 : 12}
              style={{ position: 'relative' }}
            >
              <CustomDatePicker
                index={index + 1}
                date={index == 0 ? dateFrom : dateTo}
        ...
        return numOfInputs.map((item) => {
          return (
            <Grid item lg={5}>
              ...
              <Text fontWeight={400} marginLeft='0px' fontSize='13px'>
                {item}
              </Text>
            </Grid>
          );
      ...
      case 'select':
        return numOfInputs.map((item) => {
          return (
            <CustomSelectWithArrow
              value={'12345'}
              options={[{ key: 'sfsdfsdf', value: 'sadfasdfasdf' }]}
            />
          );
        });
    }
    ...
    <>
      <DropDownTitle index={index}>
        ...
        >
          <div>{t(title)}</div>
          {inputType !== 'header' &&
          !openFilter.find((value) => value === title) ? (
      ...
      {openFilter.find((value) => value === title) && (
        <div
          style={{
            maxWidth: '400px',
            background: index % 2 !== 0 ? '#F0F1FD' : 'white',
          }}
        >
          <Flex
