...
const CustomReusableTable: React.FC<IProps> = ({ selectedRow, rowsFontSize, headers, rows, withPagination,
    totalCount, page, handlePageChange, handleRowClick }) => {
                ...
                <Table className={classes.table}>
                    ...
                    <TableBody style={{ fontSize: `${rowsFontSize}px` || "16px" }}>
                        {rows?.map((item: any, rowIndex: any) => {
                            return <TableRow
                                style={selectedRow > 0 ? { background: selectedRow === item.id ? "#A3ABEB" : undefined } : {}}
                                className={classes.tableRow} onClick={() => handleRowClick(item?.id)}>
