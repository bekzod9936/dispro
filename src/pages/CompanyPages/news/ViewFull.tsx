...
const ViewFull: React.FC<IProps> = ({ selectedSingleNews, setSelect, setModalIsVisible, setStatus, status }) => {
    ...
    const [dateFrom, setDateFrom] = useState<any>(moment(Date.now()).format("MM/DD/YYYY"));
    const [dateTo, setDateTo] = useState<any>(moment(Date.now()).format("MM/DD/YYYY"));
    ...
    return (
        <div style={{ flexGrow: 1, width: "100%" }}>
            <SettingsWrapper style={{ width: "95%", height: "95%", boxSizing: "border-box", padding: "30px 40px" }}>
                    ...
                    <>
                        <div>
                            <form onSubmit={handleSubmit(onFormSubmit)}>
                                {selectedSingleNews.image ? <div style={{ position: "relative", width: "50%" }}>
                                    <div>
                                    ...
                                    <div style={{ position: "relative", width: "fit-content" }}>
                                        ...
                                        <div
                                            ...
                                            style={{
                                                width: "280px", marginTop: "15px  ", height: "170px", borderRadius: "14px",
                                                filter: hover ? "brightness(0.7)" : "brightness(1)",
                                                overflow: "hidden"
                                            }}>
                                            <img src={selectedSingleNews.image} style={{ width: "100%", height: "100%" }} alt="" />
                                ...
                                <Flex margin="0px" flexGrow="1" justifyContent="start" alignItems="flex-start">
                                    <Flex flexDirection="column" width="50%" alignItems="flex-start" justifyContent="space-between">
                                        <Controller
                                            control={control}
                                            name="title"
                                            render={({ field }) => {
                                        ...
                                        <Controller
                                            ...
                                            name="description"
                                            render={({ field }) => {
                                                return <CustomTextArea field={field} label={t("description")} />
                                            }}
                                        />
                                    </Flex>
                                    <Flex margin="0px" flexDirection="column" width="50%" alignItems="flex-start" justifyContent="space-between">
                                        ...
                                        <Flex width="86%" justifyContent="start" margin="0px">
                                            <CustomDatePicker date={dateFrom}
                                                prefix={t("from")}

                                                handleDateChange={(date: any) => { setDateFrom(date) }}
                                            />
                                            <CustomDatePicker
                                                prefix={t("to")}
                                                date={dateTo}
                                                handleDateChange={(date: any) => { setDateTo(date) }}
                                            />

                                        </Flex>
                                        <div>
                                            {dateError && <Text fontSize="13px" fontWeight={400} color="red">{t("requiredField")}</Text>}
                                        </div>
                                        <div style={{ width: "100%" }}>
                                            <Controller
                                                control={control}
                                                name="gender"
                                                render={({ field }) => {
                                                    return <CustomReusableSelect field={field} label="chose_gender" options={options} />
                                                }}
                                ...
                                <Flex margin="20px 0px 0px 0px" justifyContent="start">
                                    <CustomButton background="white" onClick={() => setStatus("")}>
                                        <CancelIcon />
                                        <Text marginLeft="10px">{t("cancel")}</Text>
                                    </CustomButton>
                                    {status === "reset_news" ? <CustomButton type="submit">
                                        <ResetIcon />
                                        <Text marginLeft="10px" color="white">{t("resetNews")}</Text>
                                    </CustomButton> : <CustomButton type="submit">
                                        <SaveIcon />
                                        <Text marginLeft="10px" color="white">{t("save")}</Text>
                                    </CustomButton>}
                                </Flex>
                            </form>
                        </div>
                ...
                {file && <CropImageModal setIsCropModalVisible={setIsModalVisible} isCropModalVisible={isCropModalVisible} src={file} />}
            </SettingsWrapper>
