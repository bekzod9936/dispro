...
export const FullSideBar = ({ onClose, edit, rePublish, refetch }: IProps) => {
  ...
  const handleRestore = () => {
    const path = isCoupon
      ? 'create_republishcoupon'
      : 'create_republishcertificate';
    ...
    <Wrapper>
      ...
      <div>
        <Header>
          ...
          <div className='content'>
            ...
            <div className='subContent'>
              ...
              <h5>
                {currentCoupon.type === 1 ? t('certificate') : t('coupon')}
              </h5>
            </div>
          </div>
        </Header>
        <Main>
          ...
          <ul>
            <li>
              {isCoupon ? t('coupon_value') : t('certificate_value')}:{' '}
              {currentCoupon.value} {isCoupon ? '%' : 'Сум'}
            </li>
            <li>
              {isCoupon ? t('coupon_amount') : t('certificate_amount')}:{' '}
              {currentCoupon.count} шт
            </li>
            <li>
              {isCoupon ? t('coupon_price') : t('certificate_price')}:{' '}
              {currentCoupon.price} Сум
            </li>
            <li>
              {t('categories')}:{' '}
              {categories.defaults.map((category: any, index: number) => (
            ...
            <>
              ...
              <Stats>
                <p className='first'>
                  ...
                  {getPercentage(
                    currentCoupon.count,
                    currentCoupon.count - currentCoupon.stat.total
                  )}
                  %)
                </p>
                <p className='second'>
                  ...
                  {getPercentage(
                    currentCoupon.count - currentCoupon.stat.total,
                    currentCoupon.stat.used
                  )}
                  %)
                </p>
                <p className='third'>
                  Просрочено: {currentCoupon.stat.expired} (
                  {getPercentage(
                    currentCoupon.count,
                    currentCoupon.stat.expired
                  )}
                  %)
                </p>
                <p className='fourth'>
                  Не продано: {currentCoupon.stat.total} (
                  {getPercentage(currentCoupon.count, currentCoupon.stat.total)}
                  %)
                </p>
              </Stats>
            </>
      ...
      <Footer>
        {edit ? (
          ...
          <>
            <Button onClick={handleRestore} endIcon={<ReUseIcon />}>
              {t('restore')}
            </Button>
          </>
        ) : (
          <Button
            ...
            margin={{ mobile: '0 8px 0 0' }}
            endIcon={
              width > 325 && <DeleteIcon style={{ height: 15, width: 13.5 }} />
            }
            buttonStyle={{ color: '#ffffff', bgcolor: '#FF5E68' }}
          >
            {t('delete')}
