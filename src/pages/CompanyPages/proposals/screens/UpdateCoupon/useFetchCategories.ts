...
export const useFetchCategories = async (setCategories: (arg: any) => void, currentCategories: number[]) => {
    const companyCategories = useAppSelector((state: RootState) => state.info.data?.categories)
    ...
    const _ = useQuery("fetchCategories", () => fetchCategories(), {
        ...
        onSuccess: (data: any) => {
            ...
            const allowedCategories = arr.filter((el: any) => companyCategories?.includes(el.id))
