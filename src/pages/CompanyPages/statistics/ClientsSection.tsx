...
const ClientsSection = ({ filters }: any) => {
  ...
  const clientStatistics: any = useAppSelector(
    (state) => state.statistics.clientStatistics
  );
  const filterIsOpen = useAppSelector(
    (state) => state.clientStatistics.filterIsOpen
  );
            ...
            {clientStatistics?.map((item: any) => {
              return (
                <Grid
                  ...
                  spacing={0}
                  style={{
                    paddingLeft: item.title === 'ageAvg' ? '5px' : '32px',
                  }}
                >
