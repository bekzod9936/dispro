...
import {
  ...
  setFilterIsOpen,
  setStartDate,
} from '../../../services/redux/Slices/clientStatistics';
...
const OperationsSection: React.FC<IProps> = ({ filters }) => {
  ...
  const operationStatistics: any = useAppSelector(
    (state) => state.statistics.operationStatistics
  );
  ...
  const response = useQuery(
    [
      'clientStatistics',
      section,
      statisticsState.applied,
      statisticsState.startDate,
      statisticsState.endDate,
    ],
    () =>
      fetchClientStatistics(
        section,
        statisticsState.gender,
        statisticsState.purchaseAmountFrom,
        statisticsState.purchaseAmountTo,
        statisticsState.purchaseCost,
        statisticsState.trafficProvider,
        statisticsState.status,
        statisticsState.dateTo,
        statisticsState.dateFrom,
        statisticsState.startDate,
        statisticsState.endDate
      ),
    {
      ...
      onSuccess: (data) => {
        if (Icons) {
          ...
          const readyData = Object.keys(responseData)
            .splice(0, 15)
      ...
      enabled:
        !!statisticsState.applied ||
        statisticsState.endDate !== null ||
        statisticsState.startDate !== null,
      ...
      {operationStatistics.length > 0 && (
        <SectionWrapper>
          ...
          >
            <UnderSectionButton
              onClick={() => {
                dispatch(setFilterIsOpen(!statisticsState.filterIsOpen));
              }}
            >
            ...
            <UnderSectionButton>
              ...
              <div>
                ...
                <div>
                  <Text fontSize='12px' color='silver' fontWeight={400}>
                    {(statisticsState.startDate
                      ? moment(statisticsState.startDate)
                          .locale('ru')
                          .format('ll') + '-'
                      : '') +
                      `${
                        statisticsState.endDate
                          ? moment(statisticsState.endDate)
                              .locale('ru')
                              .format('ll')
                          : ''
                      }`}
              ...
              <CustomDatePickerForUndersection
                ...
                handleDateChange={handleDateChangeClientStatistics}
                isOpen={open}
                top={300}
                left={190}
                date={date}
          ...
          {inlineFilterState && (
            <InlineFilters
              ...
              filterItems={inlineFilterState}
            />
          )}
          {statisticsState.filterIsOpen && (
            <Filter
              onApply={() => dispatch(setApplied(true))}
              filters={filters}
            />
          )}
