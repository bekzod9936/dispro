    ...
    return (
        <Flex justifyContent="start" margin="0px 0px 15px 0px" >
            {Object.keys(filterItems).filter((item) => {
                ...
                return <InlineFilterWrapper style={{ marginRight: "10px", marginTop: "10px" }} >
                    <Text marginRight="0px" marginLeft="10px" fontSize="14px" fontWeight={300}> {Array.isArray(filterItems) ? t(filterItems[item]) : t(item)}</Text>
                    {!Array.isArray(filterItems) ? (<Text marginLeft="0px" marginRight="15px" fontSize="14px" fontWeight={300}> : {
                        item.toLocaleLowerCase().includes("date") ? moment(filterItems[item]).format("YYYY-MM-DD") : t(filterItems[item])}</Text>) : null}
