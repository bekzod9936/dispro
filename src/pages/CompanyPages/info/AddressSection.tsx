...
const AddressSection: React.FC<IAddressSection> = ({ setSection }) => {
  ...
  const [withRest, setWithRest] = useState([
    'mon',
    'tue',
    'wed',
    'the',
  ...
  const handleMapClick = (e: any) => {
    ...
    Geocode.fromLatLng(lat, lng).then((response: any) => {
  ...
  const handleRestChecked = (
    e: any,
    checked: boolean,
    item: string,
    index: number
  ) => {
    let workingDay = workingHoursState.find(
      (item: any) => item.day === index + 1
    );
      ...
      if (workingHoursState.includes(item)) {
        ...
        for (let i = 0; i < WORKING_DAYS.length; i++) {
          let workingHoursObject = {
            day: i + 1,
            dayOff: false,
            wHours: { from: currentWHoursS, to: currentWHoursE },
            bHours: { from: currentBHoursS, to: currentBHoursE },
          };
          newWorkingHoursState = [...newWorkingHoursState, workingHoursObject];
        }
        setWorkinHoursState(newWorkingHoursState);
  ...
  useEffect(() => {
    let searched = [...listOfAdresses]?.filter((item: any) =>
      item.address.toLowerCase().match(searchValue.toLowerCase().trim())
    );
    if (searchValue !== '') {
  ...
  return (
    <>
      <form onSubmit={handleSubmit(onFormSubmit)}>
        ...
        >
          {!listOfAdresses || currentFilial || addFilialState === 'new' ? (
            <LeftWrapper>
              <Flex
                ...
                {inputListState.map((item: any, index) => {
                  return (
                    <>
                      ...
                      />
                      {errors[item.name + index]?.type === 'required' && (
              ...
              <div
                style={{ marginLeft: '7.5%', marginTop: '10px' }}
              ...
              >
                ...
                >
                  ...
                  <div style={{ marginTop: '13px' }}>
                    ...
                    />{' '}
                    <Text fontSize='16px' fontWeight={300} marginLeft='10px'>
                      {t('24/7')}
                    </Text>
                  </div>
                  {!aroundTheClock && (
                    ...
                    >
                      {days.map((item, index) => (
                        ...
                        >
                          <div
                            onClick={(e) => handleDayCLick(e, item, index)}
                            style={{
                              marginRight: '15px',
                              display: 'flex',
                              justifyContent: 'center',
                              ...
                              >
                                ...
                                >
                                  <RadioGroup>
                                    <div>
                                      <Radio
                                        checked={checkForRest(item, index)}
                                        value='rest'
                                        onChange={(e, checked) =>
                                          handleRestChecked(
                                            e,
                                    ...
                                    <>
                                      ...
                                      <>
                                        <div>
                                          ...
                                          <span
                                            onClick={() =>
                                              handleRemoveRestClick(item, index)
                                            }
                                            style={{
                          ...
                          workingHoursState[index].wHours?.to ? (
                            ...
                            >
                              <div>{workingHoursState[index].wHours?.from}</div>
                              ...
                              <div>{workingHoursState[index].wHours.to}</div>
                            </div>
                          ) : (
                            <div
                              style={{
                  ...
                  <CustomButton style={{ marginTop: '20px' }} type='submit'>
                    <SaveIcon />
                    <Text marginLeft='18px' color='white'>
                      {t('save')}
                    </Text>
                  </CustomButton>
                </Flex>
            ...
            >
              ...
              >
                {listOfAdresses.map((item: any) => {
                  return (
                    <div
                      onClick={() => handleCompanyAddressClick(item)}
                      style={{
                        width: '100%',
                        padding: '15px 25px',
                        display: 'flex',
              ...
              >
                ...
                >
                  <Text fontSize='18px' fontWeight={400}>
                    Добавьте фотографии вашей компании для большей лояльности
                    клиентов в разделе фотографии.
                  </Text>
                </div>
                <div
                  style={{
                ...
                >
                  ...
                  <CustomButton onClick={() => setSection('photos')}>
                    <Text color='white'>Добафить фото</Text>
                  </CustomButton>
                </div>
              </Flex>
            </ModalComponent>
