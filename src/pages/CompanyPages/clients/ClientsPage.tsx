import { Avatar, Checkbox, Grid, Input } from '@material-ui/core';
import moment from 'moment';
import React, { useEffect, useState } from 'react';
import { useTranslation } from 'react-i18next';
import { useQuery } from 'react-query';
import { CancelIcon, CoinsIcon, CoinsIconWhite, QRIcon, TickIconButton } from '../../../assets/icons/ClientsPageIcons/ClientIcons';
...
const ClientsPage = () => {
    ...
    const headers: any = ["check", "client", "DiscountSum", "PointSum", "CashbackSum", "gender", "age"];
    const extraHeaders: any = ["purchuase_amount", "paid", "recomendations", "traffic_providers", "level", "last_purchase"];
    const totalHeaders: any = [...headers, ...extraHeaders].filter(item => item !== "check");
    ...
    const filters: IClientStatisticFilter[] = [
        {
            ...
            inputHandler: (value: any) => { dispatch(setClientGender(value)) },
    ...
    const handleCheckboxChange = (event: React.ChangeEvent<HTMLInputElement>, checked: boolean, id: number) => {
        event.stopPropagation();
        const client: any = response?.data?.data?.data?.clients.find((item: any) => item === id);
    ...
    const handleAddColumn = (e: React.ChangeEvent<HTMLInputElement>, checked: Boolean) => {
        if (checked) {
    ...
    const handleClientClick = (id: number) => {
        console.log(id);
        let client = response?.data?.data?.data?.clients.find((item: any) => item.id === id)
        if (client) {
            dispatch(setClient(client));
        }
    ...
    const addColumns = totalHeaders.map((item: string) => {
        return <Flex margin="0px" justifyContent="start" alignItems="center" >
            <ThemeProvider theme={theme}>
                <Checkbox color="primary" checked={clients.clientsHeaders.includes(item)} value={item} onChange={(e, checked) => handleAddColumn(e, checked)} />
            </ThemeProvider>

            <Text marginLeft="18px" fontSize="15px" fontWeight={400}>{t(item)}</Text>
        </Flex>
    })
    return (
        <>
            {!persCard ? (<PageWrapper>
                ...
                <Grid container lg={12}>
                    <Grid container lg={7}>
                        ...
                        <Flex justifyContent="space-between" alignItems="center" width="fit-content" margin="0px">
                            ...
                            <UnderSectionButton onClick={() => { setInvite(!invite) }}>
                                <QRIcon />
                                <Text>{t("invite")}</Text>
                            </UnderSectionButton>
                        </Flex>
            ...
            </PageWrapper >) : <PersonalCardClient />}
        </>
    );
}
