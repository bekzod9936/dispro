...
export const DownModal = ({ handleClose, modalContent, onClick, isBlocked, id, refetch, onClose }: IProps) => {
            ...
            {modalContent === "other" ?
                    ...
                    if (index === 0) {
                        return (
                            <p
                                ...
                                key={el.action}
                                onClick={!isBlocked ? () => onClick(el.action) : undefined}>
                                {el.label}
                            </p>)
                    }
                    else {
                ...
                modals[modalContent]?.map((el: any) =>
                    <p
                        className={isBlocked ? "disabled" : ""}
                        key={el.action}
                        onClick={isBlocked ? undefined : () => onClick(el.action)}>
                        {el.label}
                    </p>)}
        </Modal>
    )
