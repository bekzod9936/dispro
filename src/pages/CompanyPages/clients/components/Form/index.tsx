...
export const MobileForm = ({
  ...
  const { mutate, isLoading } = useMutation(
    (data: any) => changeVipPercent(data),
    {
      onSuccess: () => {
      ...
      <Wrapper onSubmit={handleSubmit}>
        <UpSide>
          <Header>
            {action <= 3 ? (
              ...
              <h2>{client.isBlocked ? t('unBlock') : t(content.title)}</h2>
            )}
            <IconButton onClick={onClose}>
              <CloseIcon />
            </IconButton>
          </Header>
          {content.subtitle && action <= 3 ? (
            <p className='subtitle'>{t(content.subtitle)}</p>
          ) : content.subtitle && !client.isBlocked ? (
            <p className='subtitle'>{t(content.subtitle)}</p>
          ) : null}
          <ClientInfo>
            {selectedClients.length > 1 ? (
              t('selectedClients') + ': ' + selectedClients.length
            ) : (
              <>
                ...
                {client.isBlocked ? (
                  ...
                  <b>
                    {action <= 2
                      ? t('points') + ': ' + client.points
                      : t('status') +
                        ': ' +
                        (client.currentStatus === client.prevStatus
        ...
        <Buttons>
          {content.additionalButton &&
            client.prevStatus !== client.currentStatus &&
            selectedClients.length <= 1 && (
              <Button
          ...
          <div>
            ...
            {action !== 4 ? (
              ...
              <Button
                type='submit'
                endIcon={
                  client.isBlocked ? <UnBlockIcon /> : <BlockWhiteIcon />
                }
                buttonStyle={{
                  color: '#ffffff',
