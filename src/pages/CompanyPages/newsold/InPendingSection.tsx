...
import {
  ExpandIcon,
  ResetIcon,
} from "../../../assets/icons/NewsIcons/NewsIcons";
import { useQuery } from "react-query";
import { fetchPendingNews } from "../../../services/queries/newPageQuery";
...
import { LimitedParagraph } from "./style";
...
const InPendingSection: React.FC<IProps> = ({
  setIsModalVisible,
  isModalVisible,
  setStatus,
  section,
  setSelectedSingleNews,
  selectedSingleNews,
}) => {
  ...
  const response = useQuery(
    ["news", section, refetch],
    () => fetchPendingNews(section),
    {
      ...
      onSuccess: (data) => {
        ...
        let formated = data?.data?.data.map((item: any) => {
          return {
            ...
            publishDate: `${moment(item.cretedAt)
              .locale("ru")
              .format("DD MMM YYYY")} - ${moment(item.endLifeTime)
              .locale("ru")
              .format("DD  MMM YYYY")}`,
            lastColumn: (
              <div
              ...
              >
                {item.pushUp ? (
                  <Tooltip
                    title={"Тут будет находится статистика поpush-уведомлениям"}
                    arrow
                    placement="left"
                    color="white"
  ...
  return (
    <div style={{ width: "100%", height: "100%" }}>
      {!tableData?.length ? (
        <NoNews setStatus={setStatus} />
      ) : (
        <>
          <CustomReusableTable
            selectedRow={selectedNew}
            handleRowClick={(id: number) => setSelectedNews(id)}
            rowsFontSize={14}
            rows={tableData}
            headers={headers}
          />
        ...
        >
          <div>
            ...
            <InfoComponent
              ...
              gender={
                selectedSingleNews.genderType === 0 ? "forMales" : "forFemale"
              }
              publishDate={`${moment(selectedSingleNews.createdAt)
                .locale("ru")
                .format("DD MMM")} - ${moment(selectedSingleNews.endLifeTime)
                .locale("ru")
                .format("DD  MMM YYYY")}`}
            />
          </div>
